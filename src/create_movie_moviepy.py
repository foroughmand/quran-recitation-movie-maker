...
audio_clip_array = audio_clip.to_soundarray()
...
def finalize(chunk_start_time, chunk_end_time, bg_image_clip, text_clips, chunk_index, extra_clips):
    ...
    # fps = audio_subclip.fps
    # audio_subarray = audio_clip_array[int(chunk_start_time*fps):int(chunk_end_time*fps + 0.01 * fps),:]
    # audio_subclip2 = AudioArrayClip(audio_subarray, fps=fps).with_duration(chunk_end_time - chunk_start_time + 0.01)
    # audio_subarray2 = audio_subclip2.to_soundarray()

    # audio_subarray3 = audio_clip_array[int(chunk_start_time*fps):(int(chunk_start_time*fps)+audio_subarray2.shape[0])]
    # audio_subclip3 = AudioArrayClip(audio_subarray3, fps=fps).with_duration(chunk_end_time - chunk_start_time)
    # print(f'X audio_subarray: {audio_clip_array.shape} {audio_subarray.shape} {audio_subarray2.shape} {audio_subclip2.start:.10f}-{audio_subclip2.end:.10f} {chunk_start_time:.10f}-{chunk_end_time:.10f}')

    # debug_eq_test = np.array([np.all(audio_subarray2[i,:] == audio_subarray[i,:]) for i in range(min(audio_subarray2.shape[0], audio_subarray.shape[0]))])
    # print(f'X {debug_eq_test} {debug_eq_test.sum()}/{debug_eq_test.shape} {debug_eq_test.argmin()} {debug_eq_test.argmax()}')
    
    # audio_subclip = audio_subclip2

    # print(f'D audio_array: {audio_array.shape}')
    ...
    if (video_range is None) or (video_range[0] <= chunk_index and chunk_index <= video_range[1]):
        video.write_videofile(fn, codec='libx264', fps=args.fps, audio_codec='aac') #DEBUG fps

    # audio_subclip.close()
    # bg_image_clip_chunk.close()
    print(f'Finalizing chunk={chunk_index}, duration={audio_subclip.duration:.2f}s, text_clips={len(text_clips)} time={chunk_start_time}-{chunk_end_time} fn={fn}')
    # for t in text_clips:
